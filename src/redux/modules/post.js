...
const initialPost = {
  ...
  // },
  image_url:
    "http://storage.enuri.info/pic_upload/knowbox2/202009/0841147752020092540507c61-d3bc-4f18-a8ff-cd36a240ca2f.jpg",
  contents: "",
  comment_cnt: 0,
  ...
  return function (dispatch, getState, { history }) {
    ...
    if (_image === _post.image_url) {
      postDB
        ...
        .update(post, layout)
        .then((doc) => {
          dispatch(editPost(post_id, { ...post }, layout));
          history.replace("/");
        });
      ...
      _upload.then((snapshot) => {
        snapshot.ref
          ...
          .then((url) => {
            postDB
              .doc(post_id)
              .update({ ...post, image_url: url, layout })
              .then((doc) => {
                dispatch(editPost(post_id, { ...post, image_url: url, layout }));
                history.replace("/");
              });
          })
          .catch((err) => {
...
export default handleActions(
  {
    ...
    [DELETE_POST]: (state, action) =>
      produce(state, (draft) => {
        let new_post_list = draft.list.filter((p) => p.id !== action.payload.post_id);
